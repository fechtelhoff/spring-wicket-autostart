...
public final class TestReportGenerator {
	...
	public static void print(String[] args) throws IOException, UnsupportedPlatformException {
		...
		if (EnumPlatform.getCurrentPlatform().getOs().isLinux()) {
			try {
				String version = (new BufferedReader(new InputStreamReader((new ProcessBuilder("lsb_release", "-ds")).start()
			...
			try {
				String name = (new BufferedReader(new InputStreamReader((new ProcessBuilder("sw_vers", "-productName")).start()
					.getInputStream()))).readLine();
				String version = (new BufferedReader(new InputStreamReader((new ProcessBuilder("sw_vers", "-productVersion")).start()
					.getInputStream()))).readLine();
